...
async function init() {
    try {
        ...
        testState.answers = testState.questions.map(q => ({
            ...
            answer: null,
            marked: false,
            timeSpent: 0 // Placeholder for per-question timing
        }));
...
function loadQuestion(index) {
    ...
    if (question.options) {
        question.options.forEach((opt, optIndex) => {
            ...
            const isSelected = currentAnswerData.answer === letter;
...
function renderNavDots() {
    ...
    testState.questions.forEach((_, i) => {
        ...
        const isAnswered = status.answer !== null;
...
function recordAnswer(index, value) {
    testState.answers[index].answer = value;
